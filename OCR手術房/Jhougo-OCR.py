...
class Frame ( wx.Frame ):
    def __init__( self, parent ):
        wx.Frame.__init__(self, parent, id = wx.ID_ANY, title = u"重頭到尾跑一遍", pos = wx.DefaultPosition, size = wx.Size(700,150))
...
def omocr3(img_name,path1,co):#ex:593-072_*****_20150508124944_199_TYC008
  #  print "C"
    ...
    if isExists :
        ...
        spup=filter(str.isdigit, spup)
        day2 = day[0:8]
        # day2 = img_name[14:20]+day[len(day)-2:len(day)]#日期年月抓檔案名稱日抓OCR辨識
        if day[0] == "2":
            day2 = int(day2)-19110000
        time2 = time[0:4]
        spup2 = spup[0:3]
...
def omred(img_name,path1,co):
   # print "F"
    ...
    if isExists :
        ...
        time2 = time[0:4]
        f = file(strsp+'\\'+img_name+'.txt', 'w+')
        f.write(img_name+u';'+str(day2)+u';'+str(time2)+u';') # write text to file
        if ispath :
            f.write(content)#寫入抓到的照相地點
        else:
            f.write(' ; ')
